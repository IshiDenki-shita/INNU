...
public:
    ...
    // 目標方向(theta)へ直接メカナムで進むduty比を計算する（出力はしない）
    // theta: ロボット正面(x軸)を基準とした目標方向 [rad]
    //        反時計回りが正、theta=0で前進、theta=pi/2で左移動
    void DutyUpdate(float theta) {
        float vx = std::cos(theta);
        float vy = std::sin(theta);

        // メカナム標準式（回転項なし、目標方向へ直接並進）
        float fl = vx - vy;
        float fr = vx + vy;
        float rl = vx + vy;
        float rr = vx - vy;
...
int main(int argc, char** argv) {
    ...
    ros::Rate rate(50);
    while (ros::ok()) {
        motors.DutyUpdate(purpose_theta_g);
        motors.PWMshootor();
