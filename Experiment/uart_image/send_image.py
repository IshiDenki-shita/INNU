"""
UARTがPythonのコードに組み込まれるイメージのコード
"""
...
class BallDetection:
    ...
    def run(self, red) -> Tuple[float, float, bool, int, int]:
        """
        1フレーム分の検出〜位置計算をまとめて行う。
        戻り値: (angle, distance, detected, x, y)
        detected=Falseのときangle/distanceは無効値。x/yは表示用(draw_detection_overlay向け)。
        """
        ...
        if (x, y) == (-1, -1):
            return 0.0, -1.0, False, -1, -1  # ロスト時: angle, distance は無効値

        theta, distance = self.calc_ball_position((x, y), S)
        return theta, distance, True, x, y
...
if __name__ == "__main__":
    ...
    cam.start_camera()
    uart.start()
    logger.debug("Camera and UART started")
    ...
    try:
        while True:
            ...
            theta, distance, detected, x, y = ball.run(clean)
            uart.send(angle=theta, distance=distance, detected=detected)
